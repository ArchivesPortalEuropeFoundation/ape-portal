jQuery(function($) {
    ...
    var templates = {
        ...
        modalImageView: '<div class="image-large">' +
            '   <div class="mgimage-image-toolbar">' +
            '       <button class="mgimage-image-toolbarbtn mgimage-rotate-counter" aria-label="<%= moreGallery.lang("rotate_counter_clockwise") %>" title="<%= moreGallery.lang("rotate_counter_clockwise") %>"><i class="icon icon-reply"></i></button>' +
            '       <button class="mgimage-image-toolbarbtn mgimage-rotate-clockwise" aria-label="<%= moreGallery.lang("rotate_clockwise") %>" title="<%= moreGallery.lang("rotate_clockwise") %>"><i class="icon icon-share"></i></button></button>' +
            '   </div>' +
    ...
    moreGallery.ImageAppView = Backbone.View.extend({
        ...
        modelStartEdit: function(data) {
            ...
            var $toolbar = $('.mgimage-image-toolbar');
            $('.mgimage-rotate-counter, .mgimage-rotate-clockwise').on('click', function(e) {
                e.preventDefault();
                var btn = $(this),
                    rotation = btn.hasClass('mgimage-rotate-counter') ? 'counter' : 'clockwise';
                btn.attr('disabled', true);
                btn.html('<i class="icon icon-refresh icon-spin"></i>');

                $toolbar.addClass('mgimage-image-toolbar__visible');
                data.model.set('rotate', rotation);
                data.model.save(null, {
                    success: function (model, response, options) {
                        data.model.trigger('startEdit', {model: data.model});
                    }
                });
            });
