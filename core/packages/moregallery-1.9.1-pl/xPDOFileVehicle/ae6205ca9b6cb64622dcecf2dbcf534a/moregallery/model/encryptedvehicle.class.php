    ...
    protected function decodePayloads(&$transport, $action = 'install')
    {
        if (isset($this->payload['object_encrypted']) || isset($this->payload['related_objects_encrypted'])) {
            ...
            if (isset($this->payload['object_encrypted'])) {
                $decoded = $this->decode($this->payload['object_encrypted'], $key);
                if ($decoded === false) {
                    $transport->xpdo->log(xPDO::LOG_LEVEL_ERROR, 'Failed to decode object with key ' . $key);
                    return false;
                }
            ...
            if (isset($this->payload['related_objects_encrypted'])) {
                $decoded = $this->decode($this->payload['related_objects_encrypted'], $key);
                if ($decoded === false) {
                    $transport->xpdo->log(xPDO::LOG_LEVEL_ERROR, 'Failed to decode related objects with key ' . $key);
                    return false;
                }
